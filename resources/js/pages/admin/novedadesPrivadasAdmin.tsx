...
import NovedadPrivadaAdminRow from '@/components/novedadPrivadaAdminRow';
...
export default function NovedadesPrivadasAdmin() {
    ...
    const handleSubmit = (e) => {
        ...
        post(route('admin.novedadesprivadas.store'), {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Novedad privada creada correctamente');
                reset();
                setText('');
                setCreateView(false);
            },
            onError: (errors) => {
                toast.error('Error al crear novedad privada');
                console.log(errors);
            },
        });
    };
        ...
        <Dashboard>
            <div className="flex w-full flex-col p-6">
                ...
                <div className="mx-auto flex w-full flex-col gap-3">
                    <h2 className="border-primary-orange text-primary-orange text-bold w-full border-b-2 text-2xl">
                        Novedades Privadas
                    </h2>
                    ...
                    <div className="flex w-full justify-center">
                        <table className="w-full border text-left text-sm text-gray-500 rtl:text-right dark:text-gray-400">
                            ...
                            <tbody className="text-center">
                                {novedadesPrivadas.data?.map((novedadPrivada) => (
                                    <NovedadPrivadaAdminRow key={novedadPrivada.id} novedadPrivada={novedadPrivada} />
                                ))}
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div className="mt-4 flex justify-center">
                        {novedadesPrivadas.links && (
                            <div className="flex items-center">
                                {novedadesPrivadas.links.map((link, index) => (
                                    <button
                                        key={index}
                                        ...
                                        } ${index === 0 ? 'rounded-l-md' : ''} ${index === novedadesPrivadas.links.length - 1 ? 'rounded-r-md' : ''}`}
                    ...
                    <div className="mt-2 text-center text-sm text-gray-600">
                        Mostrando {novedadesPrivadas.from || 0} a {novedadesPrivadas.to || 0} de {novedadesPrivadas.total}{' '}
                        resultados
                    </div>
                </div>
            </div>

...
export default function NovedadPrivadaAdminRow({ novedadPrivada }) {
    const [edit, setEdit] = useState(false);
    const [text, setText] = useState(novedadPrivada?.text || '');
    ...
    const handleUpdate = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        updateForm.post(route('admin.novedadesprivadas.update'), {
            preserveScroll: true,
            onSuccess: () => {
                toast.success('Novedad privada actualizada correctamente');
                setEdit(false);
            },
            onError: (errors) => {
                toast.error('Error al actualizar novedad privada');
                console.log(errors);
            },
        });
    };

    const deleteNovedadPrivada = () => {
        if (confirm('¿Estas seguro de eliminar esta novedad privada?')) {
            updateForm.delete(route('admin.novedadesprivadas.destroy'), {
                preserveScroll: true,
                onSuccess: () => {
                    toast.success('Novedad privada eliminada correctamente');
                },
                onError: (errors) => {
                    toast.error('Error al eliminar novedad privada');
                    console.log(errors);
                },
            });
        }
    ...
    return (
        <tr className="border text-black odd:bg-gray-100 even:bg-white">
            ...
            <td className="align-middle">
                <div dangerouslySetInnerHTML={{ __html: novedadPrivada?.text }}></div>
            </td>

            <td className="h-[90px] w-[90px] px-8">
                <img className="h-full w-full object-contain" src={novedadPrivada?.image} alt="" />
            </td>
            ...
            <td className="w-[140px] text-center">
                <div className="flex flex-row justify-center gap-3">
                    <button
                        ...
                        className="h-10 w-10 rounded-md border border-blue-500 px-2 py-1 text-white"
                    >
                        <FontAwesomeIcon icon={faPen} size="lg" color="#3b82f6" />
                    </button>
                    <button
                        onClick={deleteNovedadPrivada}
                        className="h-10 w-10 rounded-md border border-red-500 px-2 py-1 text-white"
                    >
                            ...
                            <div className="w-[500px] rounded-md bg-white p-4">
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <input
                                        ...
                                        name="type"
                                        id="type"
                                        defaultValue={novedadPrivada?.type}
                                    ...
                                    <input
                                        ...
                                        name="nombree"
                                        id="nombree"
                                        defaultValue={novedadPrivada?.title}
